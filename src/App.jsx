...
export default function App() {
  ...
  const [speechRate, setSpeechRate] = useLocalStorage('hanzi:speechRate', 0.8);
  const { voices, selectedVoiceURI, setSelectedVoiceURI, speak } = useVoices(speechRate);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col font-sans overflow-x-hidden text-[14px]">
      ...
      <Header viewMode={viewMode} onChangeMode={setViewMode} onOpenSettings={() => setShowSettings(true)} />

      {showSettings && (
        <SettingsModal
          voices={voices} selectedVoiceURI={selectedVoiceURI} onSelectVoice={setSelectedVoiceURI}
      ...
      <main className="flex-1 flex flex-col lg:flex-row p-3 lg:p-8 gap-5 lg:gap-8 max-w-[1600px] mx-auto w-full items-start">
        <aside className="w-full lg:w-[380px] lg:sticky lg:top-24 lg:max-h-[calc(100vh-120px)] lg:overflow-y-auto custom-scrollbar bg-white p-5 lg:p-6 rounded-[2rem] lg:rounded-[2.5rem] shadow-sm border border-slate-200 print:hidden shrink-0 flex flex-col gap-5">
          ...
          {viewMode === 'stroke' && <StrokeSidebar data={strokeData} onChange={setStrokeData} />}
          {viewMode === 'pinyin' && <PinyinSidebar data={pinyinData} onChange={setPinyinData} pinyinResult={pinyinResult} />}
          {viewMode === 'radicals' && <RadicalsSidebar search={radicalSearch} onSearchChange={setRadicalSearch} />}
        </aside>
